...
void child_handler(Dataset *training, Dataset *testing, int K, 
    ...
    //TODO
    int start_idx,N,n,ans,correct;
    correct = 0;
    if((n = read(p_in, &start_idx, sizeof(int))) == -1) {
        perror("read");
        exit(1);
        } 
        else if(n == 0) {
    ...
    //fprintf(stderr, "Start index is %d, N is %d \n",start_idx,N);
    for (int i=0;i<N;i++){
        ans = knn_predict(training,&(testing->images[start_idx+i]),K,fptr);
        if (ans == testing->labels[start_idx+i]){
            correct ++;
        }
    }
    //fprintf(stderr,"correct is %d\n",correct);
...
double distance_cosine(Image *a, Image *b){
    ...
    for (int i =0;i<x;i++){
        for (int j=0; j<y;j++){
            sum1 += a->data[i*x+j]*b->data[i*x+j];
            sum2 += a->data[i*x+j]*a->data[i*x+j];
            sum3 += b->data[i*x+j]*b->data[i*x+j];
        }
    }
    return 2*acos(sum1/(sqrt(sum2)*sqrt(sum3)))/M_PI;
}
